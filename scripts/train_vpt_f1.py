...
from algorithms.foundation_dagger.vpt_model.actions import Buttons, ActionTransformer
...
def train_vpt_bc(
    ...
    def run_epoch(data_loader, train: bool):
        ...
        camera_angle_abs_sum = torch.zeros(2, dtype=torch.float64)
        camera_angle_l2_sum = 0.0
        camera_angle_count = 0
        if train:
            model.train()
        else:
            model.eval()
        ...
        for batch_idx, batch in enumerate(iterator):
            ...
            with torch.set_grad_enabled(train):
                ...
                pred_camera_np = pred_camera_bins.cpu().numpy().astype(np.int64)
                target_camera_np = target_camera_bins.cpu().numpy().astype(np.int64)
                pred_camera_cont = action_transformer.undiscretize_camera(pred_camera_np.copy())
                target_camera_cont = action_transformer.undiscretize_camera(target_camera_np.copy())
                axis_abs_diff = np.abs(pred_camera_cont - target_camera_cont)
                camera_angle_abs_sum += torch.from_numpy(axis_abs_diff.sum(axis=0)).to(torch.float64)
                camera_angle_l2_sum += np.linalg.norm(pred_camera_cont - target_camera_cont, axis=1).sum()
                camera_angle_count += axis_abs_diff.shape[0]
        ...
        if running_examples > 0:
            ...
            if camera_angle_count > 0:
                metrics["camera_mae_x"] = (camera_angle_abs_sum[0] / camera_angle_count).item()
                metrics["camera_mae_y"] = (camera_angle_abs_sum[1] / camera_angle_count).item()
                metrics["camera_mae_l2"] = camera_angle_l2_sum / camera_angle_count
        if running_examples == 0:
            return 0.0, metrics
