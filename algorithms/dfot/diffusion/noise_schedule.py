...
def cosine_simple_diffusion_schedule(
    ...
    """
    t_min = torch.atan(torch.exp(-0.5 * torch.tensor(logsnr_max, dtype=torch.float32)))
    t_max = torch.atan(torch.exp(-0.5 * torch.tensor(logsnr_min, dtype=torch.float32)))
...
def beta_linear_schedule(
    ...
    """
    betas = torch.linspace(start, end, timesteps, dtype=torch.float32)
...
def sd_schedule(timesteps, start=0.00085, end=0.0120):
    ...
    """
    betas = torch.linspace(start**0.5, end**0.5, timesteps, dtype=torch.float32) ** 2
